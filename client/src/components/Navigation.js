...
const Navigation = ({ currentTool, setCurrentTool }) => {
  return (
    <nav className="mt-4">
      <div className="flex flex-wrap bg-gray-800 rounded-xl overflow-hidden">
        <button
        ...
        <button
          className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 transition-colors ${
            currentTool === 'testCodeGenerator' 
              ? 'bg-gray-900 text-white' 
              : 'text-gray-400 hover:text-white hover:bg-gray-700'
          }`}
          onClick={() => setCurrentTool('testCodeGenerator')}
        >
          <span>Test Code Generator</span>
        </button>
      </div>
    </nav>
  );
